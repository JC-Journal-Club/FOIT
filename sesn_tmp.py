...
ses_data, ses_label = utils.load_source_data(dataset_name=dataset_name, FOIT_type='cross-session')
...
for sub_number in range(15):
    ...
    print(len(ses_data_sub))
    print(len(ses_data_sub[0]))
    print(len(ses_data_sub[1]))
    ses_data_sub = utils.normalization(ses_data_sub)
    ...
    L_S_data = utils.normalization(L_S_data)
    ...
    for j in range(len(weight_for_clfs)):
        proba_result_all += clf_sources[j].predict_proba(ud_data) * weight_for_clfs[j]
